...
- [ ] Add STOP/prohibition block to `.claude/commands/build-feature.md` immediately after the YAML
...
- [ ] Write `.claude/hooks/inline_work_guard.py`:
...
- [ ] Update `.claude/commands/build-feature.md` to write `.build-feature.lock` at the very
      start of argument resolution and clean it up on all exit paths (success, zero-match,
      multi-match, error).
- [ ] Update `.claude/skills/build-single-ticket/SKILL.md` to delete `.build-feature.lock`
      in Step 1 (before dispatching `ticket-supervisor`) so phase agents are not blocked.
- [ ] Update `.claude/agents/epic-supervisor.md` Pre-Flight Reads to delete
      `.build-feature.lock` at the start of execution (before any ticket-supervisor spawn)
      so phase agents are not blocked.
- [ ] Register the hook in `.claude/settings.json` under `PreToolUse` with matcher
      `Edit|Write`, following the same `bash -c 'd="$PWD"; while ...'` repo-root-walk
      pattern used by existing hooks.

### test-writer

- [ ] `test_blocks_edit_when_lock_exists`: create a temp lock file; send an Edit payload
      via stdin; assert exit code 2 (in block mode).
- [ ] `test_allows_edit_when_no_lock`: no lock file present; send an Edit payload; assert
      exit code 0.
- [ ] `test_jsonl_audit_log_written`: trigger a block; assert JSONL record exists with
      required fields (`timestamp`, `tool_name`, `file_path`, `session_id`).
- [ ] `test_warn_mode_exits_zero`: set `INLINE_WORK_GUARD_MODE=warn`; trigger would-be
      block; assert exit code 0 AND JSONL record written.
- [ ] `test_exception_failopen`: simulate an exception (e.g. malformed stdin); assert
      exit code 0.
- [ ] Place tests in `unit_tests/commit_guardian/test_inline_work_guard.py`.
