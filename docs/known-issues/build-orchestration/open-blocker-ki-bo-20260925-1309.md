...
**Same family as.** This is one more instance of one pattern: the driver's reply contract
...
- [KI-BO-20260901-1045](open-blocker-ki-bo-20260901-1045.md) — the driver routes handoffs on
  `handoff_target`. Its remaining scope lists 19 templates that still never mention the field,
  and both agents that failed here, `documentation-expert` and `ac-fulfillment-gate`, are on it.
  That entry also says it is unverified whether the engine enforces the schema's `if`/`then`;
  the error above (`must match "then" schema`) shows that it does.
- [KI-BO-20260901-1052](resolved/resolved-high-ki-bo-20260901-1052.md) — resolved by BO-3000a
  (#687): `python-coder` and `test-writer` now tell the agent to return `handoff_target`.
- [KI-BO-20260907-0851](open-low-ki-bo-20260907-0851.md) — two templates use `handoff` to mean
  "stop, I need a human".
- [KI-BO-20260907-0850](resolved/resolved-high-ki-bo-20260907-0850.md) — resolved by #687:
  `build-ticket.js` had drifted from its twin. Both mitigations below change both drivers
...
also close the remaining scope of KI-BO-20260901-1045 and KI-BO-20260907-0851. It needs ACs through
