...
**Two more instances of this class, both found 2026-08-19.** The proposed fix ("walk
...
- **Hand-placed files are indistinguishable from deployed ones.**
  `.leafcutter/config/doc_types.json` and `diagram_types.json` are present in this workspace
  and deployed by no build phase; someone copied them in as a workaround for KI-BP-003. Since
  `.leafcutter/` is gitignored and nothing reconciles it against the manifest, that workaround
  is invisible **and** it masks the deploy gap it was working around, so the defect tests as
  fixed here. Same root cause as the orphan, opposite origin: this file arrived from outside
  the build rather than being abandoned by it.
...
### KI-BP-008 — `.claude/skills/` is symlinked wholesale to the generated tree, so an adopter's own skills have nowhere to live and `--clean` targets them
...
**A second, independent defect in the same function** — a doubled path segment that has made
clean-mode's `workflows` entry unreachable since it was added, so a real orphan survives every
`--clean`. Filed separately as **KI-BP-009**. Sequence the two deliberately: fixing BP-009
activates a code path that this entry shows is unsafe wherever adopters also write.
...
### KI-BP-009 — Clean-mode's `workflows` entry has a doubled path segment, so it has never run and a real orphan survives every `--clean`
...
when the total is zero. The manifest side is fine: `_build_source_manifests`
(`scripts/build.py:1246-1258`) does populate a `workflows` key from
`templates/workflows-js/*.js`. The entry is correctly configured and simply never consulted.
...
`--clean` while the run reports success. This is a live instance of KI-BP-005 (a deployed
file the source no longer has) that the mechanism intended to catch it cannot see.

Found while verifying KI-BP-008, not by any failure report — nothing surfaces it, because a
no-op cleanup and a genuinely clean tree produce identical output.

**Relationship to KI-BP-008.** Same function, same `--clean` invocation, distinct defects
with distinct fixes: BP-008 is about *whose files* clean-mode is entitled to touch; this is
about a path it cannot reach. Sequence them deliberately — repairing this one activates a
code path that BP-008 shows is unsafe for any directory an adopter also writes into. Workflows
are not currently such a directory, so the two are separable here, but only by accident of
which trees adopters happen to use.
