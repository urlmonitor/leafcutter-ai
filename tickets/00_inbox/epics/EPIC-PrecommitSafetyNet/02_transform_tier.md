...
- [ ] Run `create-hook` skill twice to scaffold both transform hooks:
...
- [ ] Implement `transform_doc_frontmatter.py`:
...
- [ ] Implement `transform_description_field.py`:
...
- [ ] Add `tier` field to all existing `hooks_manifest` entries that are missing it.
