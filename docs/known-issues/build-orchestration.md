...
| `high` | KI-BO-017 — A fast-lane re-run whose worktree was pruned silently rebuilds on the old branch tip instead of the latest `origin/main` | [resolved-high-ki-bo-017.md](build-orchestration/resolved/resolved-high-ki-bo-017.md) |
| `high` | KI-BO-022 — A CRLF acceptance-criterion record is rewritten LF end-to-end by a single `work_status` flip, and every value-level check still passes | [resolved-high-ki-bo-022.md](build-orchestration/resolved/resolved-high-ki-bo-022.md) |
