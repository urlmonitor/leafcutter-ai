...
### build_ac_store
A `build.py` deployment phase (`build_ac_store()` in `scripts/build_phases.py`) that copies the six AC pipeline scripts from the leafcutter-ai source tree into the consumer project's `scripts/ac_store/` directory. Added in EPIC-AcPipelineDeployGaps to close the portability gap between the `ac-scanner` and `build-ac` skills (marked `portable: true` in `skill_registry.json`) and their dependency scripts. Without this phase running, the skills are deployed as SKILL.md files but their scripts are absent, causing runtime failures. See ADR-013 for the canonical definition of `portable: true`.
