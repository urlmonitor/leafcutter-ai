...
def test_zero_dispatch_script_fails_guard() -> None:
    ...
    Uses build-epic.js as the canonical E1-only example: it defines
    ...
    assert result.error == "", (
        f"Harness error while running build-epic.js: {result.error}"
    )
    # This is the key assertion for AC-2: build-epic.js dispatches 0 agents.
    # If this fails (dispatch_count > 0), build-epic.js has been ported to E2
    # and the xfail marker in test_e2_dispatch_count should be removed.
    assert result.dispatch_count == 0, (
        f"build-epic.js unexpectedly dispatched {result.dispatch_count} agents "
