...
**The fix, written and verified on 2026-09-23, then withheld.** The docs-only branch should
return the normalised `out_of_scope` set instead of a bare empty one:
...
That is a narrowing of the guard, not a removal, and `BP-1100e-1-iii` is preserved unchanged
in effect. A docs-only ticket that declares **nothing** out of scope still yields an empty
declared scope, so it still cannot silently absorb source changes it never mentioned — the
case that guard was written for. A source file named in neither list is still flagged.

It was covered by `unit_tests/commit_guardian/test_predone_scope_docs_only_out_of_scope.py`
with three cases: the one that was broken, the one that must stay strict, and
`BP-1100e-1-iii`'s own. The middle one is load-bearing — a "fix" that simply deleted the
