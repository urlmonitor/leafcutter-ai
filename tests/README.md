...
- Mutate the copy the test **imports**. Commit-guardian modules exist twice (source under
  `templates/`, build output under `scripts/`) and which one a given test loads is a
  per-file choice, not a repo-wide default — mutating the wrong one **silently does nothing
  and reports green**, indistinguishable from the dead test you are hunting. See
  `unit_tests/README.md` §1a for the procedure to determine which copy a test loads before
  you mutate it.
...
Broadly, `tests/` holds the wider-scope suites, organized as a mix of topic subdirectories
(currently `tests/ac_store/`, `tests/commit_guardian/`, `tests/knowledge/`,
`tests/testing_quality/`) and roughly four dozen top-level `test_*.py` modules that have not
been grouped into a subdirectory, plus `tests/fixtures/` and a `conftest.py`. This list of
subdirectory names will rot as new ones are added — the structural fact that holds is: check
both the topic subdirectories **and** the top-level modules for an existing home before
adding a new file, and put yours beside its nearest sibling either way.
...
**Build first — this is not optional.** `scripts/commit_guardian/` (and
`scripts/doc_compliance/`, `scripts/feedback/`) are build output: symlinks into
`.leafcutter/scripts/…` created by `build.py`'s `install_shims()` step (ADR-016). They do
not exist on a fresh clone or worktree. Tests import from those paths at startup, so
skipping this step does not fail loudly — collection errors for the missing modules are
swallowed by `--continue-on-collection-errors` below, and you are left looking at a
green-looking but silently truncated run.

```bash
python scripts/build.py --target-dir .                    # required first — see above

python -m pytest tests/ -q                                 # this root
AC_ENFORCE_STRICT=1 python -m pytest tests/ unit_tests/ -q --continue-on-collection-errors
                                                             # what CI actually runs (see below)
```
