---
title: "goal_to_epic.py is decomposed into fourteen modules, and thirteen uncovered ACs gain test contracts"
date: "2026-09-21"
time: "14:30"
type: manual
components:
...
### Thirteen uncovered ACs gain test contracts
...
widening a hook skip. Five describe tests that already exist, citing the real
function names and each directory's actual framework; eight are forward
specifications for behaviour not yet built.

**The tests themselves are deliberately not in this change.** They were written
(19 failed / 9 passed for the TKT set, 2 failed / 5 passed for the ACD set
under `AC_ENFORCE_STRICT=1` — a correct red baseline against unimplemented
behaviour) and then withheld: CI runs strict AC enforcement, so
`pytest_ac_enforcement` does not xfail-mask a not-done AC's tests there, and a
red baseline landing on `main` would fail the required pytest gate for everyone.
The repo's design is that tests arrive with their implementation. These eight
should land in that shape, against the contracts this change records.
...
Locally the full suite was never run end-to-end — roughly six hours serially,
and long runs were repeatedly killed under load — so an explicit blast-radius
set was used instead (719 passed, 0 failed after the final rebase).

**CI closed that gap.** PR #844's run executed the whole suite in 38 minutes:
**6207 passed, 11 skipped, 7 xfailed**, with the only failures being the 21
withheld red tests described above. No third-party consumer install was
simulated beyond the repo's own `Consumer install simulation (BP-900h-1)`
check, which passed.
