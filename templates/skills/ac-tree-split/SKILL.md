...
6. **Redistribute L2s.** For each L2 being moved:
   - Update its `depends_on` to reference the new L1 ID
   - Remove the old L1 ID from `depends_on`
...
9. **GATE: Blast radius preview.**
   Count and list all modified files. Agent checks and makes a suggestion.
   **User can override.**

10. **Validate.** Run the AC validator to confirm:
...
|-------|----------------------|------------------------|-------------------------------|
| `depends_on` | Update moved L1s: old L0 -> new L0. L2s under moved L1s are **unaffected** (L1 IDs don't change). | Update moved L2s: old L1 -> new L1. | All references change (IDs change). |
| `delivers_to` / `expects_from` | **No change.** Contracts reference AC IDs, not parent IDs. L1 IDs are stable. | **No change** unless L2 IDs change (they don't in v1). | Must rewire all contracts (IDs change). |
| `covered_by` | Remove from old parent, add to new parent. | Remove old L1 from L0, add new L1s to L0. | Remove from old component, add to new component. |
| `superseded_by` | **Not set.** Original L0 is trimmed, not superseded. | Set on original L1 pointing to new L1 siblings. | Set on original component root. |
