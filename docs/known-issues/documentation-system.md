...
The adopter-delivery half is filed separately as **KI-DS-002** — writing the four
conventions into `docs/` does not put them where an adopter's agents will look.
...
Writing them is necessary and not sufficient — they also have to reach an adopter. See
KI-DS-002.
...
so an adopter's `docs/how-to/documentation/` is empty on a fresh install.
...
installed, for a different reason, with an identical symptom — which is the kind of gap that
gets closed twice and reported three times.
...
Whichever ships, note that the fix lands in the **build pipeline** while the defect presents in
the documentation system — the two known-issue registers meet here, and a fix filed under only
one of them will look complete from that side.
