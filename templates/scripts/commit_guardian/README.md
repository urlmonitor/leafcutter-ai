...
- **Porting to Another Project**: Copy the `scripts/commit_guardian/` folder. Edit `commit_guardian.json` to match the new project's structure. Update `root_files.allowed_files`, `excluded_dirs`, and any hardcoded path patterns — `check_root_files.py`'s allowlist in particular still carries values from the repository this folder was extracted from, and will block a commit touching a root file it does not name. See `scripts/commit_guardian/INTEGRATION.md` _(forthcoming)_ for the full porting checklist.
