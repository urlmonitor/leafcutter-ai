...
description: |
  ...
  the full prioritized ready list, dispatches ticket-supervisors directly (one
  per ready ticket, grouping epic tickets into a shared worktree), and loops
  until the backlog is empty or the user halts. Supports --dry-run, --limit N,
  --epic-only, and --ticket-only flags.
portable: true
signoff: false
domain: null
adopter_notes: |
  ...
  Warning: this command dispatches ticket-supervisors directly and can build
  multiple items in sequence without further confirmation. Use --dry-run
...
| `--dry-run` | Print the full ordered ready list and exit; do NOT dispatch any ticket-supervisor |
...
Exit 0. Do NOT dispatch any ticket-supervisor or worktree-agent.
...
#### Step 4b — Dispatch ticket-supervisor directly

> **Rationale:** `/build-feature` is NOT called here. Claude Code's hard
> sub-agent depth limit means build-backlog → /build-feature → ticket-supervisor
> → phase agents would exceed depth 2. Instead, this command dispatches
> ticket-supervisor directly at depth 0, keeping phase agents at depth 1.
> See ADR-006 / EPIC-FlattenSupervisorChain for the full design rationale.

**For epic items** (path contains an `EPIC-*` directory component):

1. Call `worktree-agent create` with the first sub-ticket path to get or create
   the epic worktree:

   {% if platform == 'claude' %}
   ```
   Agent(agent="worktree-agent", input={action: "create", ticket_path: "<first sub-ticket path>"})
   ```
   {% elif platform == 'antigravity' %}
   ```bash
   python .agents/agents/worktree-agent/scripts/run.py --action="create" --args="<first sub-ticket path>"
   ```
   {% endif %}

   Capture the returned `worktree_path`. If the agent reports failure, record
   the failure and skip this item (do not halt the entire loop unless the error
   is structural).

2. Read the epic's `Master_Plan.md` and every sub-ticket frontmatter to compute
   `ready_batch` — the maximal set of tickets that:
...
3. Dispatch one `ticket-supervisor` per ticket in `ready_batch` in parallel via
   the `Agent` tool:

   {% if platform == 'claude' %}
   ```
   Agent(agent="ticket-supervisor", input={ticket_path: "<absolute path to sub-ticket>"})
   ```
   {% elif platform == 'antigravity' %}
   ```bash
   python .agents/agents/ticket-supervisor/scripts/run.py --ticket="<absolute path>"
   ```
   {% endif %}

   After issuing all dispatches, confirm all result blocks appear in context
   before proceeding (dispatch verification).

4. Route on results:
   - `done` → ticket fully signed off; continue.
   - `blocked` (local to this ticket) → record payload, continue with remaining items.
   - `blocked` (structural, affects other tickets) → halt loop, surface all pending payloads.
   - `failed` → record failure, continue or halt per user preference (Step 4b outcome handling below).

**For standalone items** (no `EPIC-*` directory component in path):

Delegate to the `build-single-ticket` sub-skill via the `Skill` tool. This
sub-skill owns the inbox → todo → done lifecycle and spawns `ticket-supervisor`
internally:

{% if platform == 'claude' %}
```
Skill(skill="build-single-ticket", args="<CURRENT_ITEM.path>")
```
{% elif platform == 'antigravity' %}
```bash
python .agents/skills/build-single-ticket/scripts/run.py "<CURRENT_ITEM.path>"
```
{% endif %}

**Outcome handling (both epic and standalone):**
...
On failure (ticket-supervisor or build-single-ticket returned `failed` or `blocked`):

1. Record the failure:
   ```
   failures.append({path: CURRENT_ITEM.path, error: <blocker_summary from payload>})
   ```
...
#### Step 4c — Re-evaluate the ready list
...
| ticket-supervisor returns `failed` or `blocked` | Prompt user to continue or halt; record failure. |
| worktree-agent reports failure | Record failure for that epic item; prompt user to continue or halt. |
...
(ticket-supervisor runs here via build-single-ticket, producing its own output)
...
(worktree-agent creates/reuses worktree; ticket-supervisor(s) dispatched directly)
