...
3. Categorizes commits using rules from `{{config.changelog_categories_path}}` (if the file exists); falls back to conventional-commit prefix heuristics when absent
4. Reads `docs/components.json` to select relevant registered component IDs
5. Writes a new per-file entry to `{{config.changelog_folder}}` via `emit_entry.py`
6. Does NOT create a deployment tag (that is the job of `/prod-deploy`)
7. Does NOT write to or modify the legacy `CHANGELOG.md`

## Output Format

Each entry is a Markdown file at `{{config.changelog_folder}}YYYY-MM-DD-HHMM-<slug>.md`:
...
Categorization rules are loaded from `{{config.changelog_categories_path}}`
