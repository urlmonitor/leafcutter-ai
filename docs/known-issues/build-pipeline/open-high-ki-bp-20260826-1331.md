...
**Re-verified 2026-09-23:** STILL TRUE. The fix direction's core ask — a source-revision
stamp so `build.py` can refuse to overwrite a deployed artifact with one built from an
ancestor commit — has not landed: read the whole workflow-install phase
(`scripts/build_phases_workflows.py`) end to end and grepped every `build_phases*.py`
module for `commit`, `revision`, `ancestor`, `git rev-parse`, and `HEAD`; the only hits
are an unrelated clean-mode orphan-provenance ledger (KI-BP-20260921-1630's subject, which
only ever learns about artifacts the *current* build still produces, not a source-commit
comparison) and a product-truth smoke-test's own "regression fixture" naming. The install
phase still writes whenever content differs, with no notion of which revision either side
came from — exactly the mechanism this entry describes. Also checked for the two cheaper
mitigations named in Fix direction: no pre-flight parity check exists at the
`/build-feature` or fast-lane entry points (searched for `parity_check`,
`preflight_parity`, `route_regression` — no hits), and per-worktree output roots are not
in use (the shared-symlink topology this entry documents is still how this worktree
itself is deployed — `.leafcutter` here resolves to `/home/henzeh/projects/leafcutter/
.leafcutter`, confirmed via `ls -la`). Affects `/plan-feature`: incidentally —
`plan-feature.js` appears only as one row (64 drifted lines, the smallest in the original
table) in a drift table spanning all nine deployed workflows; the mechanism (a shared,
last-writer-wins `.leafcutter/`) is generic to the whole build-pipeline component and is
not specific to plan-feature in any way this entry argues.
