...
def _bootstrap(main_repo: Path, worktree_path: Path) -> None:
    ...
    **Pre-commit config safety net** (``_establish_pre_commit_config``):
    ensures ``.leafcutter`` or ``.pre-commit-config.yaml`` is present in
    the worktree root even if the build.py step was skipped or failed.  This
    is the authoritative guarantee that pre-commit hooks will not silently skip
