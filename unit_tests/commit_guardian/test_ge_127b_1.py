...
EXERCISE STRATEGY (per CLAUDE.md "Gate / Workflow ACs -- Verify
    ...
    scripts/build.py mutated by a single in-place, non-quote-touching edit
    that leaves both the raw and the docstring-stripped line count identical
    -- the survivability proof the keystone record exists to make true.
    (Originally scripts/build_phases.py, cited by the AC as "largest:
    scripts/build_phases.py at 2603 counted / 3873 raw" -- that file was
    subsequently split by the build_phases.py size-limit refactor and is now
    340 counted lines, UNDER the 400-line limit, so it no longer exercises
    this test's premise. scripts/build.py is ~1915 counted lines and remains
    genuinely, tracked-ly oversized; repointed here.)
...
# scripts/build.py -- genuinely oversized post the build_phases.py size-
# limit split (see module docstring EXERCISE STRATEGY for why it replaces
# scripts/build_phases.py, now under-limit at 340 counted lines).
