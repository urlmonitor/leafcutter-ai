...
RED BASELINE (as authored, before the removal — kept for the record):
    - templates/workflows-js/fast-lane-build.js still existed on disk, so
      build_workflow_scripts() (globbing templates/workflows-js/*.js) still
      copied it to <target>/.leafcutter/workflows/fast-lane-build.js. Test 1
      and Test 3 assert its ABSENCE from that deployed directory and were
      therefore RED.
    - Test 2 asserts that the checked-in orphan source file itself no longer
      ...
      was therefore RED for the same reason.

NOW GREEN. The removal landed with this file; all three pass under
AC_ENFORCE_STRICT=1. The baseline above is retained because it records what
these assertions were able to detect when the orphan was present — a test that
has only ever been green cannot show that.

GREEN (after the removal, per the AC's test_spec):
...
def test_deployed_layout_has_no_orphaned_fast_lane_runner(tmp_path):
    ...
    Was RED before the removal: the orphan still existed, so the real build
    still deployed it to <output_root>/workflows/fast-lane-build.js.
    """
...
def test_no_command_or_workflow_routes_to_the_removed_runner(tmp_path):
    ...
    Was RED before the removal: the orphan still existed on disk, so the AC's
    literal "it is removed from the package" clause was unsatisfied and the
    first assertion below failed.
    """
...
def test_injection_builders_still_deployed_after_the_removal(tmp_path):
    ...
    Was RED before the removal: the orphan still existed, so the "orphan
    absent" precondition failed even though injection_builders.py was already
    ...
    # output_root IS the deployed .leafcutter directory (see _run_real_build),
    # and agent-support scripts land under it, not beside it. Verified against a
    # clean build: .leafcutter/scripts/injection_builders.py exists and
    # <target>/scripts/injection_builders.py does not.
    deployed_injection_builders = output_root / "scripts" / "injection_builders.py"
