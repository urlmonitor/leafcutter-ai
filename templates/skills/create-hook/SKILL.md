...
name: create-hook
description: >
  Scaffold a new pre-commit hook fully embedded in the leafcutter
  package. A single invocation creates the hook script template, registers the
  hook in commit_guardian.json (config key + hooks_manifest entry), and adds a
  row to the hook documentation index. Invoked by workflow-architect.
