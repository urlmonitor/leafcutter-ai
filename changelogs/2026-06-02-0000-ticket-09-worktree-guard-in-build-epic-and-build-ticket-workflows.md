...
description: "PR #30. build-epic.js and build-ticket.js now check at the top of run() whether .git is a directory (main clone) — if so, execution halts immediately with { status: 'error', worktree_required: true } before any agent is dispatched. build-epic.js also propagates worktree_path to all workflow('build-ticket') sub-calls. create-ticket.js receives a JSDoc comment confirming it intentionally runs on main. 5 new unit tests across test_build_epic_workflow.py and test_build_ticket_workflow.py, all passing."
...
| File | Change |
|------|--------|
| `templates/workflows-js/build-epic.js` | Worktree guard + `worktree_path` propagated to sub-workflow calls |
| `templates/workflows-js/build-ticket.js` | Worktree guard at top of `run()` |
