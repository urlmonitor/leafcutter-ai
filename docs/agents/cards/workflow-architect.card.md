...
- BP-600a-1: Quick-fix operates in the current worktree when it can, and self-isolates when it cannot
- BP-600a-2: Quick-fix never uses worktree-agent, the feature skill, or a hand-rolled git worktree add
...
- BP-600b-1: Quick-fix writes a schema-complete AC into the hierarchical AC store and back-links its parent
- BP-600b-1-i: Quick-fix workflow detects when an equivalent AC already exists
- BP-600b-2: Quick-fix AC id is a next unused child suffix under its parent, in the matched component
- BP-600b-2-i: Quick-fix workflow infers component from diagnosed file path when not provided
- BP-600b-3: Quick-fix AC persists after the run ends, however the run ends
- BP-600c-1: Quick-fix workflow dispatches test-writer to create a failing test before the fix
- BP-600c-2: Quick-fix confirms the test fails (red phase) under strict AC enforcement
...
- BP-600d-3: Quick-fix commits child AC, parent AC, test and fix together in one commit
- BP-600d-4: Quick-fix pushes its commits to origin and closes the run
...
- BP-600f: Quick-fix self-isolates off the default branch instead of committing to it
