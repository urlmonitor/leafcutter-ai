...
| `requires_adr` | **required** | Tri-state: `true` (ADR needed), `false` (considered, not needed), `null` (not applicable — pre-existing coverage). **Absent key is a hook failure** per ADR-026. **Set `requires_adr: true` when the ticket modifies any of the following supervisor/agent contract surfaces** (emerged from EPIC-CompletionManifestSignoff KI-2): (1) the `ticket-supervisor`'s phase-agent dispatch loop or status-tag routing; (2) the signoff skill's format, status enum, or validation rules (`templates/skills/signoff/SKILL.md`); (3) the `building-epics` skill's supervisory actions or retry caps; (4) any agent template's contract with its supervisor — e.g. expected comment format, `completion_manifest` schema, or required frontmatter fields. These are system-boundary changes whose rationale, alternatives, and migration path must be recorded in an ADR so future maintainers can understand why the contract changed. |
