...
| `failed` | One or more real schema or cross-reference errors were found (schema violation, broken AC pointer, phantom-done AC without implementation evidence, derived-data drift, etc.). | Something concrete is wrong; the process exits `1` and the pre-commit gate blocks the commit. | That the *rest* of the record is sound — errors are collected across every check in one run, so `failed` does not imply the run stopped early or that every other check passed; it implies at least one thing is provably wrong. |
